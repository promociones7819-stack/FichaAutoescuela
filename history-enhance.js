(() => {
  ...
  style.textContent = `
    ...
    .dateField{grid-column:auto}.dateQuick{display:flex;gap:6px;flex-wrap:wrap}.dateQuick button{min-height:36px;padding:6px 10px;font-size:12px;background:#fff;color:var(--blue2);border:1px solid var(--line)}
    .dateQuick button.primaryDate{background:var(--light);border-color:#8bcbe0}.dateReadable{font-size:12px;color:var(--muted);min-height:18px}
  ...
  function offset(days){const d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()+days);return iso(d)}
  ...
  function enhance(){
    ...
    const quick=document.createElement('div');quick.className='dateQuick';
    [['Ayer',-1],['Hoy',0],['Mañana',1]].forEach(([text,days])=>{const b=document.createElement('button');b.type='button';b.textContent=text;if(days===0)b.classList.add('primaryDate');b.addEventListener('click',()=>{date.value=offset(days);date.dispatchEvent(new Event('change',{bubbles:true}));});quick.append(b)});
    dateBox.append(quick);
